...
| **OpenAIP Core API** | VFR zones + special-use airspace polygons with vertical limits (optional, BYOK) | Community-maintained; [OpenAIP](https://www.openaip.net) — not for primary flight planning; attribution required | "Airspace data © openAIP contributors" |
...
- **OpenAIP airspaces.** The VFR airspace layer is optional and BYOK: without `OPENAIP_API_KEY` the layer is empty. With a key, VFR zones and special-use airspace polygons (with vertical limits) are fetched through the server-side `/api/openaip` proxy, and the required "Airspace data © openAIP contributors" credit is registered in the Data attribution popover. OpenAIP data is community-maintained and must not be used for primary flight planning.
