...
[[ -n "${OPENAIP_API_KEY:-}" ]] && KEY_SETUP_EXTERNAL_KEYS+=(OPENAIP_API_KEY)
...
OPENAIP_API_KEY="${OPENAIP_API_KEY:-$(read_dotenv_value "OPENAIP_API_KEY")}"
...
OPENAIP_API_KEY="${OPENAIP_API_KEY:-$(read_keychain_secret "openaip-api" "api-key")}"
...
case "${HOST}" in
  ...
  *)
    ...
    echo "!! OpenSky, AISStream, TomTom, FIRMS, OpenAIP, LL2, Google) to ANYONE who can"
...
[[ -n "${OPENAIP_API_KEY}" ]] && echo "OpenAIP key (VFR airspaces): configured" || echo "OpenAIP key (VFR airspaces): not set — airspace layer requires a key"
...
put_env_if_set OPENAIP_API_KEY "${OPENAIP_API_KEY}"
