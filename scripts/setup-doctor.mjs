...
export const CREDENTIALS = Object.freeze([
  ...
  { name: 'OPENAIP_API_KEY', label: 'OpenAIP airspace', keychain: [['openaip-api', 'api-key']] },
...
export function buildCapabilitySummary(credentials) {
  ...
  return {
    ...
    airspaces: configured('OPENAIP_API_KEY') ? 'live OpenAIP airspace feed' : 'off until an OpenAIP key is added',
...
export function formatSetupReport(report, { readyMessage } = {}) {
  ...
  const lines = [
    ...
    `Airspaces: ${report.capabilities.airspaces}`,
